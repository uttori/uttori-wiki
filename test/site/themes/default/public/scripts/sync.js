...
// Page script that handles the syncing functionality for the Sync page
const SyncPage = function SyncPage($scope) {
  this.$scope = $scope;
...
SyncPage.prototype = {
  ...
  onCancelClick(e) {
    e.preventDefault();
    $.each(this.requests, (index, request) => {
      request.abort();
    });
  },
  ...
  // Makes a request to the target server to get a document based on the given slug
  requestSingle(slug) {
    const endpoint = `${this.syncUrl}/${slug}`;

    const request = $.get(endpoint, this.requestSingleResponse.bind(this))
      .fail(() => {
  ...
  writeSingle(document) {
    ...
    // make the request
    const request = $.post(writeUrl, document, this.writeSingleResponse.bind(this))
      .fail(() => {
  ...
  // Handles the single write response back from the server
  writeSingleResponse(data) {
...
$(() => {
  const page = new SyncPage($(SELECTORS.PAGE_CONTAINER));
});
